...
class Control:
    ...
    subprocess.Popen(
        shlex.split(check_startup), stdout=subprocess.PIPE, universal_newlines=True
    )
    ...
    def unmute_microphone():
        script = 'set volume input volume 100'
        Script_execution.run(script)
...
class Get_data:
    ...
    def current_all():
        ...
        return json.dumps(
            {"track": track, "artist": artist, "album": album}, ensure_ascii=False
        )
...
class Script_execution:
    ...
    def run(script_to_execute):
        script = (
            f"osascript -e 'tell application \"Music\"\n{script_to_execute}\nend tell'"
        )
        subprocess.Popen(
            shlex.split(script), stdout=subprocess.PIPE, universal_newlines=True
        )

    # Run osascript and return itune data to function
    def get(script):
        acquisition_data, _ = subprocess.Popen(
            shlex.split(script), stdout=subprocess.PIPE, universal_newlines=True
        ).communicate()
